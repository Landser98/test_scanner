...
cors_origins_raw = os.environ.get("ALLOWED_ORIGINS", "")
cors_origins = [x.strip() for x in cors_origins_raw.split(",") if x.strip()]
if not cors_origins:
    ui_port = os.environ.get("UI_PORT", "8502").strip() or "8502"
    ui_services_raw = os.environ.get("UI_SERVICE_NAMES", "bank-parser-upload-ui,bankparserui")
    ui_services = [x.strip() for x in ui_services_raw.split(",") if x.strip()]
    cors_origins = [f"http://{service}:{ui_port}" for service in ui_services]
