...
def safe_open_file(file_path: Union[str, Path], mode: str = 'r', 
    ...
    return open_validated_path(validated_path, mode=mode, **kwargs)


def open_validated_path(validated_path: Path, mode: str = 'r', **kwargs):
    """Open an already validated path object."""
    if not isinstance(validated_path, Path):
        validated_path = Path(validated_path)
    return validated_path.open(mode, **kwargs)
